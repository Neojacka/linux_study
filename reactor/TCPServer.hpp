...
private:
    ...
    void recever(Connection *conn)
    {
        ...
        {
            char buff[1024];
            ssize_t n = recv(conn->_sockfd, buff, sizeof(buff), 0);
            if (n > 0)
            {
                buff[n] = 0;
                conn->_inbuff += buff;
        ...
        // 确保得到完整报文
        Util::PackageSplit(conn->_inbuff, result);
        for (auto &message : result)
        {
            // 业务处理
            _service(conn, message);
        }
        logMessage(NORMAL, "sockfd[%d]的业务处理完成",conn->_sockfd);
    }
...
public:
    TCPServer(uint16_t port, handlt_t service)
        : _port(port), _service(service), _num(defaultnum)
    {
    }
    void init()
    {
